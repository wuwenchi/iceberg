...
public class SparkZOrderStrategyV2 extends SparkSortStrategy {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(SparkZOrderStrategyV2.class);
  ...
  public SparkZOrderStrategyV2(Table table, SparkSession spark, List<String> zOrderColNames) {
    super(table, spark);
    ...
    Stream<String> identityPartitionColumns = table
        .spec()
        .fields()
        .stream()
  ...
  public Set<DataFile> rewriteFiles(List<FileScanTask> filesToRewrite) {
    ...
    try {
      ...
      switch (spatialCurveStrategyType) {
        case DIRECT:
          zOrderUDF = new DirectSparkZOrderUDF(zOrderColNames.size(), varLengthContribution, maxOutputSize);
          zValueArray = direct(zOrderColumns, zOrderUDF);
          break;

        case SAMPLE:
          zOrderUDF = new SampleSparkZOrderUDF(zOrderColNames.size(), maxOutputSize);
          zValueArray = sample(scanDF, zOrderColumns, zOrderUDF);
          break;

        default:
          throw new UnsupportedOperationException(String.format(
              "Unsupported type %s", spatialCurveStrategyType));
      }
  ...
  private Column direct(List<StructField> zOrderColumns, BaseSparkZOrderUDF zOrderUDF) {
    Column zValueArray;
    zValueArray = functions.array(zOrderColumns.stream()
        .map(colStruct -> zOrderUDF.sorted(functions.col(colStruct.name()), colStruct.dataType(), null))
        .toArray(Column[]::new));
    return zValueArray;
  }

  private Column sample(Dataset<Row> scanDF, List<StructField> zOrderColumns, BaseSparkZOrderUDF zOrderUDF) {
    ...
    for (int i = 0; i < zOrderColumns.size(); i++) {
      ...
      columns[i] = zOrderUDF.sorted(functions.col(colStruct.name()), colStruct.dataType(), broadcast.value()[i]);
    }
