...
public class TestRewriteDataFilesAction extends SparkTestBase {
  ...
  protected RewriteDataFiles basicRewrite(Table table) {
  ...
  public void testBinPackWithFilter() {
    ...
    Result result = basicRewrite(table).filter(Expressions.equal("c1", 1)).filter(Expressions.startsWith("c2", "foo")).execute();
  ...
  public void testBinPackAfterPartitionChange() {
    ...
    RewriteDataFiles.Result result = basicRewrite(table).option(SortStrategy.MIN_INPUT_FILES, "1").option(SortStrategy.MIN_FILE_SIZE_BYTES, Integer.toString(averageFileSize(table) + 1000)).option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Integer.toString(averageFileSize(table) + 1001)).execute();

    Assert.assertEquals("Should have 1 fileGroup because all files were not correctly partitioned", 1, result.rewriteResults().size());
  ...
  public void testBinPackWithDeletes() throws Exception {
    ...
    Result result = actions().rewriteDataFiles(table, tableLocation)
        ...
        .option(BinPackStrategy.MIN_FILE_SIZE_BYTES, "0").option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Long.toString(Long.MAX_VALUE - 1)).option(BinPackStrategy.MAX_FILE_SIZE_BYTES, Long.toString(Long.MAX_VALUE)).option(BinPackStrategy.DELETE_FILE_THRESHOLD, "2").execute();
    Assert.assertEquals("Action should rewrite 2 data files", 2, result.rewrittenDataFilesCount());
  ...
  public void testBinPackWithDeleteAllData() {
    ...
    table.refresh();
    List<Object[]> expectedRecords = currentData();
    Result result = actions().rewriteDataFiles(table, tableLocation).option(BinPackStrategy.DELETE_FILE_THRESHOLD, "1").execute();
    Assert.assertEquals("Action should rewrite 1 data files", 1, result.rewrittenDataFilesCount());

    List<Object[]> actualRecords = currentData();
    assertEquals("Rows must match", expectedRecords, actualRecords);
    Assert.assertEquals("Data manifest should not have existing data file", 0, (long) table.currentSnapshot().dataManifests(table.io()).get(0).existingFilesCount());
    Assert.assertEquals("Data manifest should have 1 delete data file", 1L, (long) table.currentSnapshot().dataManifests(table.io()).get(0).deletedFilesCount());
    Assert.assertEquals("Delete manifest added row count should equal total count", total, (long) table.currentSnapshot().deleteManifests(table.io()).get(0).addedRowsCount());
  }
  ...
  public void testBinPackWithStartingSequenceNumber() {
    ...
    table.refresh();
    Assert.assertTrue("Table sequence number should be incremented", oldSequenceNumber < table.currentSnapshot().sequenceNumber());
  ...
  public void testBinPackSplitLargeFile() {
    ...
    Result result = basicRewrite(table).option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Long.toString(targetSize)).option(BinPackStrategy.MAX_FILE_SIZE_BYTES, Long.toString(targetSize * 2 - 2000)).execute();
  ...
  public void testBinPackCombineMediumFiles() {
    ...
    Result result = basicRewrite(table).option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Integer.toString(targetSize)).option(BinPackStrategy.MAX_FILE_SIZE_BYTES, Integer.toString((int) (targetSize * 1.8))).option(BinPackStrategy.MIN_FILE_SIZE_BYTES, Integer.toString(targetSize - 100)) // All files too small
        .execute();
  ...
  public void testMultipleGroups() {
    ...
    // Perform a rewrite but only allow 2 files to be compacted at a time
    RewriteDataFiles.Result result = basicRewrite(table).option(RewriteDataFiles.MAX_FILE_GROUP_SIZE_BYTES, Integer.toString(fileSize * 2 + 1000)).option(BinPackStrategy.MIN_INPUT_FILES, "1").execute();
  ...
  public void testPartialProgressMaxCommits() {
    ...
    // Perform a rewrite but only allow 2 files to be compacted at a time
    RewriteDataFiles.Result result = basicRewrite(table).option(RewriteDataFiles.MAX_FILE_GROUP_SIZE_BYTES, Integer.toString(fileSize * 2 + 1000)).option(RewriteDataFiles.PARTIAL_PROGRESS_ENABLED, "true").option(RewriteDataFiles.PARTIAL_PROGRESS_MAX_COMMITS, "3").execute();
  ...
  public void testSingleCommitWithRewriteFailure() {
    ...
    BaseRewriteDataFilesSparkAction realRewrite = (org.apache.iceberg.spark.actions.BaseRewriteDataFilesSparkAction) basicRewrite(table).option(RewriteDataFiles.MAX_FILE_GROUP_SIZE_BYTES, Integer.toString(fileSize * 2 + 1000));
    ...
    doThrow(new RuntimeException("Rewrite Failed")).when(spyRewrite).rewriteFiles(any(), argThat(failGroup));

    AssertHelpers.assertThrows("Should fail entire rewrite if part fails", RuntimeException.class, () -> spyRewrite.execute());
  ...
  public void testSingleCommitWithCommitFailure() {
    ...
    AssertHelpers.assertThrows("Should fail entire rewrite if commit fails", RuntimeException.class, () -> spyRewrite.execute());
  ...
  public void testParallelSingleCommitWithRewriteFailure() {
    ...
    BaseRewriteDataFilesSparkAction realRewrite = (org.apache.iceberg.spark.actions.BaseRewriteDataFilesSparkAction) basicRewrite(table).option(RewriteDataFiles.MAX_FILE_GROUP_SIZE_BYTES, Integer.toString(fileSize * 2 + 1000)).option(RewriteDataFiles.MAX_CONCURRENT_FILE_GROUP_REWRITES, "3");
  ...
  public void testInvalidOptions() {
    ...
    AssertHelpers.assertThrows("No negative values for partial progress max commits", IllegalArgumentException.class, () -> basicRewrite(table).option(RewriteDataFiles.PARTIAL_PROGRESS_ENABLED, "true").option(RewriteDataFiles.PARTIAL_PROGRESS_MAX_COMMITS, "-5").execute());

    AssertHelpers.assertThrows("No negative values for max concurrent groups", IllegalArgumentException.class, () -> basicRewrite(table).option(RewriteDataFiles.MAX_CONCURRENT_FILE_GROUP_REWRITES, "-5").execute());

    AssertHelpers.assertThrows("No unknown options allowed", IllegalArgumentException.class, () -> basicRewrite(table).option("foobarity", "-5").execute());

    AssertHelpers.assertThrows("Cannot set rewrite-job-order to foo", IllegalArgumentException.class, () -> basicRewrite(table).option(RewriteDataFiles.REWRITE_JOB_ORDER, "foo").execute());
  }
  ...
  public void testSortMultipleGroups() {
    ...
    // Perform a rewrite but only allow 2 files to be compacted at a time
    RewriteDataFiles.Result result = basicRewrite(table).sort().option(SortStrategy.REWRITE_ALL, "true").option(RewriteDataFiles.MAX_FILE_GROUP_SIZE_BYTES, Integer.toString(fileSize * 2 + 1000)).execute();
  ...
  public void testSortAfterPartitionChange() {
    ...
    Assert.assertEquals("Should have 1 fileGroup because all files were not correctly partitioned", result.rewriteResults().size(), 1);
  ...
  public void testSortCustomSortOrder() {
    ...
    RewriteDataFiles.Result result = basicRewrite(table).sort(SortOrder.builderFor(table.schema()).asc("c2").build()).option(SortStrategy.REWRITE_ALL, "true").option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Integer.toString(averageFileSize(table))).execute();
  ...
  public void testSortCustomSortOrderRequiresRepartition() {
    ...
    RewriteDataFiles.Result result = basicRewrite(table).sort(SortOrder.builderFor(table.schema()).asc("c3").build()).option(SortStrategy.REWRITE_ALL, "true").option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Integer.toString(averageFileSize(table) / partitions)).execute();
  ...
  public void testAutoSortShuffleOutput() {
    ...
    RewriteDataFiles.Result result = basicRewrite(table).sort(SortOrder.builderFor(table.schema()).asc("c2").build()).option(SortStrategy.MAX_FILE_SIZE_BYTES, Integer.toString((averageFileSize(table) / 2) + 2))
        // Divide files in 2
        .option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Integer.toString(averageFileSize(table) / 2)).option(SortStrategy.MIN_INPUT_FILES, "1").execute();
  ...
  public void testCommitStateUnknownException() {
    ...
    AssertHelpers.assertThrows("Should propagate CommitStateUnknown Exception", CommitStateUnknownException.class, () -> spyAction.execute());
  ...
  public void testZOrderSort() {
    ...
    RewriteDataFiles.Result result = basicRewrite(table).zOrder("c2", "c3").option(SortStrategy.MAX_FILE_SIZE_BYTES, Integer.toString((averageFileSize(table) / 2) + 2))
        // Divide files in 2
        .option(RewriteDataFiles.TARGET_FILE_SIZE_BYTES, Integer.toString(averageFileSize(table) / 2)).option(SortStrategy.MIN_INPUT_FILES, "1").execute();
    ...
    Assert.assertTrue("Should have reduced the number of files required for c2", filesScannedC2 < originalFilesC2);
    Assert.assertTrue("Should have reduced the number of files required for c3", filesScannedC3 < originalFilesC3);
    Assert.assertTrue("Should have reduced the number of files required for a c2,c3 predicate", filesScannedC2C3 < originalFilesC2C3);
  }
  ...
  public void xzwTest() {
    ...
    List<StructField> zOrderColumns = Arrays.stream(rowDf.schema().names()).map(rowDf.schema()::apply).collect(Collectors.toList());
    ...
    zOrderColumns = Arrays.stream(Arrays.copyOf(zOrderColumns.toArray(), zOrderColumns.size() - 1, StructField[].class)).collect(Collectors.toList());
    ...
    // 去掉不支持的类型字段
    zOrderColumns = Arrays.stream(Arrays.copyOf(zOrderColumns.toArray(), zOrderColumns.size() - 2, StructField[].class)).collect(Collectors.toList());
  ...
  public void testZOrderAllTypesSort() {
    ...
    RewriteDataFiles.Result result = basicRewrite(table).zOrder("longCol", "intCol", "floatCol", "doubleCol", "dateCol", "timestampCol", "stringCol", "binaryCol", "booleanCol").option(SortStrategy.MIN_INPUT_FILES, "1").option(SortStrategy.REWRITE_ALL, "true").execute();
  ...
  public void testInvalidAPIUsage() {
    ...
    AssertHelpers.assertThrows("Should be unable to set Strategy more than once", IllegalArgumentException.class, "Cannot set strategy", () -> actions().rewriteDataFiles(table, tableLocation).binPack().sort());

    AssertHelpers.assertThrows("Should be unable to set Strategy more than once", IllegalArgumentException.class, "Cannot set strategy", () -> actions().rewriteDataFiles(table, tableLocation).sort().binPack());

    AssertHelpers.assertThrows("Should be unable to set Strategy more than once", IllegalArgumentException.class, "Cannot set strategy", () -> actions().rewriteDataFiles(table, tableLocation).sort(SortOrder.unsorted()).binPack());
  }
  ...
  public void testRewriteJobOrderBytesAsc() {
    ...
    BaseRewriteDataFilesSparkAction basicRewrite = (BaseRewriteDataFilesSparkAction) basicRewrite(table).binPack();
    List<Long> expected = toGroupStream(table, basicRewrite).mapToLong(RewriteFileGroup::sizeInBytes).boxed().collect(Collectors.toList());

    BaseRewriteDataFilesSparkAction jobOrderRewrite = (BaseRewriteDataFilesSparkAction) basicRewrite(table).option(RewriteDataFiles.REWRITE_JOB_ORDER, RewriteJobOrder.BYTES_ASC.orderName()).binPack();
    List<Long> actual = toGroupStream(table, jobOrderRewrite).mapToLong(RewriteFileGroup::sizeInBytes).boxed().collect(Collectors.toList());
  ...
  public void testRewriteJobOrderBytesDesc() {
    ...
    BaseRewriteDataFilesSparkAction jobOrderRewrite = (BaseRewriteDataFilesSparkAction) basicRewrite(table).option(RewriteDataFiles.REWRITE_JOB_ORDER, RewriteJobOrder.BYTES_DESC.orderName()).binPack();
    List<Long> actual = toGroupStream(table, jobOrderRewrite).mapToLong(RewriteFileGroup::sizeInBytes).boxed().collect(Collectors.toList());
  ...
  public void testRewriteJobOrderFilesAsc() {
    ...
    BaseRewriteDataFilesSparkAction basicRewrite = (BaseRewriteDataFilesSparkAction) basicRewrite(table).binPack();
    List<Long> expected = toGroupStream(table, basicRewrite).mapToLong(RewriteFileGroup::numFiles).boxed().collect(Collectors.toList());

    BaseRewriteDataFilesSparkAction jobOrderRewrite = (BaseRewriteDataFilesSparkAction) basicRewrite(table).option(RewriteDataFiles.REWRITE_JOB_ORDER, RewriteJobOrder.FILES_ASC.orderName()).binPack();
    List<Long> actual = toGroupStream(table, jobOrderRewrite).mapToLong(RewriteFileGroup::numFiles).boxed().collect(Collectors.toList());
  ...
  public void testRewriteJobOrderFilesDesc() {
    ...
    BaseRewriteDataFilesSparkAction jobOrderRewrite = (BaseRewriteDataFilesSparkAction) basicRewrite(table).option(RewriteDataFiles.REWRITE_JOB_ORDER, RewriteJobOrder.FILES_DESC.orderName()).binPack();
    List<Long> actual = toGroupStream(table, jobOrderRewrite).mapToLong(RewriteFileGroup::numFiles).boxed().collect(Collectors.toList());
  ...
  private Stream<RewriteFileGroup> toGroupStream(Table table, BaseRewriteDataFilesSparkAction rewrite) {
    rewrite.validateAndInitOptions();
    Map<StructLike, List<List<FileScanTask>>> fileGroupsByPartition = rewrite.planFileGroups(table.currentSnapshot().snapshotId());

    return rewrite.toGroupStream(new RewriteExecutionContext(fileGroupsByPartition), fileGroupsByPartition);
  }

  protected List<Object[]> currentData() {
    return rowsToJava(spark.read().format("iceberg").load(tableLocation).sort("c1", "c2", "c3").collectAsList());
  }
  ...
  protected void shouldHaveSnapshots(Table table, int expectedSnapshots) {
    ...
    Assert.assertEquals("Table did not have the expected number of snapshots", expectedSnapshots, actualSnapshots);
  }

  protected void shouldHaveNoOrphans(Table table) {
    Assert.assertEquals("Should not have found any orphan files", ImmutableList.of(), actions().deleteOrphanFiles(table).olderThan(System.currentTimeMillis()).execute().orphanFileLocations());
  }
  ...
  private <T> Pair<T, T> boundsOf(DataFile file, NestedField field, Class<T> javaClass) {
    ...
    return Pair.of(javaClass.cast(Conversions.fromByteBuffer(field.type(), file.lowerBounds().get(columnId))), javaClass.cast(Conversions.fromByteBuffer(field.type(), file.upperBounds().get(columnId))));
  }
  ...
  private <T> List<Pair<Pair<T, T>, Pair<T, T>>> checkForOverlappingFiles(Table table, String column) {
    ...
    Map<StructLike, List<DataFile>> filesByPartition = Streams.stream(table.currentSnapshot().addedFiles(table.io())).collect(Collectors.groupingBy(DataFile::partition));

    Stream<Pair<Pair<T, T>, Pair<T, T>>> overlaps = filesByPartition.entrySet().stream().flatMap(entry -> {
      List<DataFile> datafiles = entry.getValue();
      Preconditions.checkArgument(datafiles.size() > 1, "This test is checking for overlaps in a situation where no overlaps can actually occur because the " + "partition %s does not contain multiple datafiles", entry.getKey());

      List<Pair<Pair<T, T>, Pair<T, T>>> boundComparisons = Lists.cartesianProduct(datafiles, datafiles).stream().filter(tuple -> tuple.get(0) != tuple.get(1)).map(tuple -> Pair.of(boundsOf(tuple.get(0), field, javaClass), boundsOf(tuple.get(1), field, javaClass))).collect(Collectors.toList());

      Comparator<T> comparator = Comparators.forType(field.type().asPrimitiveType());

      List<Pair<Pair<T, T>, Pair<T, T>>> overlappingFiles = boundComparisons.stream().filter(filePair -> {
        ...
        boolean boundsDoNotOverlap =
            ...
            (comparator.compare(rMax, lMax) >= 0 && comparator.compare(rMin, lMax) >= 0) || (comparator.compare(lMax, rMax) >= 0 && comparator.compare(lMin, rMax) >= 0);
  ...
  private void writeDF(Dataset<Row> df) {
    df.select("c1", "c2", "c3").sortWithinPartitions("c1", "c2").write().format("iceberg").mode("append").save(tableLocation);
  }
  ...
  private List<DeleteFile> writePosDeletes(Table table, StructLike partition, String path, int outputDeleteFiles) {
    ...
    for (int file = 0; file < outputDeleteFiles; file++) {
      ...
      EncryptedOutputFile encryptedOutputFile = EncryptedFiles.encryptedOutput(outputFile, EncryptionKeyMetadata.EMPTY);

      GenericAppenderFactory appenderFactory = new GenericAppenderFactory(table.schema(), table.spec(), null, null, null);
      PositionDeleteWriter<Record> posDeleteWriter = appenderFactory.set(TableProperties.DEFAULT_WRITE_METRICS_MODE, "full").newPosDeleteWriter(encryptedOutputFile, FileFormat.PARQUET, partition);
  ...
  private Set<String> cacheContents(Table table) {
    return ImmutableSet.<String>builder().addAll(manager.fetchSetIDs(table)).addAll(coordinator.fetchSetIDs(table)).build();
  }
