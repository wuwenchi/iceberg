...
@Internal
public class RowDataFileChangeLogReader extends BaseScanTaskReader<RowData, ChangelogScanTask> {
  ...
  @Override
  public CloseableIterator<RowData> open(ChangelogScanTask scanTask) {

    CloseableIterable<RowData> iterable = openChangelogScanTask(scanTask, projectedSchema());
  ...
  private CloseableIterable<RowData> openChangelogScanTask(
    ...
    if (task instanceof AddedRowsScanTask) {
      CloseableIterable<RowData> iterable =
          openAddedRowsScanTask((AddedRowsScanTask) task, requestedSchema);
      return iterable;
    } else if (task instanceof DeletedRowsScanTask) {
      throw new UnsupportedOperationException("Deleted rows scan task is not supported yet");
    } else if (task instanceof DeletedDataFileScanTask) {
      CloseableIterable<RowData> iterable =
          openDeletedDataFileScanTask((DeletedDataFileScanTask) task, requestedSchema);
      return iterable;
    } else {
      throw new IllegalArgumentException(
          "Unsupported changelog scan task type: " + task.getClass().getName());
  ...
  CloseableIterable<RowData> openAddedRowsScanTask(AddedRowsScanTask task, Schema requestedSchema) {
